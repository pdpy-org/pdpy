...
from ..classes.default import Default, IEMGuiNames
...
class XmlToJson:
  ...
  def addNodes(self, x):
    if x.tag in ['vis', 'x', 'y', 'width', 'height', 'font', 'title', 'name']:
      return
    ...
    if 'comment' == x.tag:
      self.patch.__last_canvas__().comment(
      Comment(
        x.findtext('x'), 
        x.findtext('y'),
        x.text)
    ...
    if 'connect' == x.tag:
      self.patch.__last_canvas__().edge(
        Edge( 
    ...
    __id__   = x.get('id', default=self.patch.__obj_idx__)
    __xpos__ = x.findtext('x')
    __ypos__ = x.findtext('y')
    ...
    if 'floatatom' == x.tag or 'symbolatom' == x.tag or 'listbox' == x.tag:
      ...
      self.patch.__last_canvas__().add(obj)
      return
    ...
    # end iemgui if statement -----------------------------------------------
      ...
      print('Making', x.tag)
      obj = PdObject(__id__,__xpos__,__ypos__,x.tag)
